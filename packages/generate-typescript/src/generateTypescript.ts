...
import { CodeWriter, codeWriter } from "./common"
...
export default function generate(schema: SchemaData, { global }: GenerateOptions): string {
  ...
  const td = codeWriter().line(`// Types`).line()
  const md = codeWriter().line("// Metadata").line()
...
function defineApi(schema: SchemaData, td: CodeWriter) {
...
function defineMetadata(md: CodeWriter) {
...
function exportApi(schema: SchemaData, md: CodeWriter) {
...
function writeEnums(schema: SchemaData, td: CodeWriter, md: CodeWriter) {
...
function writeComposites(schema: SchemaData, td: CodeWriter, md: CodeWriter) {
...
function writeTables(schema: SchemaData, td: CodeWriter, md: CodeWriter) {
  td.line(`// Tables`).line()
...
function writeViews(schema: SchemaData, td: CodeWriter, md: CodeWriter) {
  td.line(`// Views`).line()
  md.line(`// View Metadata`).line()
  for (const view of schema.views) {
    td.line(`/** \`${view.name}\` table. */`)
...
function writeFunctions(schema: SchemaData, td: CodeWriter, md: CodeWriter) {
...
function writeAttributes(attributes: AttributeData[], td: CodeWriter, md: CodeWriter, skipType = false) {
